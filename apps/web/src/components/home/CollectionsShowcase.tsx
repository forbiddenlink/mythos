...
import { Library, Sparkles, Skull, Heart, Swords, ArrowRight } from 'lucide-react';
...
const featuredIds = ['trickster-gods', 'underworld-rulers', 'love-deities', 'war-gods'];
    ...
    <section className="container mx-auto max-w-7xl px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-serif text-xl font-semibold">Themed Collections</h2>
        ...
        {featured.map((collection) => {
          ...
          return (
            <Link key={collection.id} href={`/collections/${collection.slug}`}>
              <Card className={`h-full bg-gradient-to-br ${colorMap[collection.id] || ''} hover:scale-[1.02] transition-transform cursor-pointer group`}>
                <CardContent className="p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="p-2 rounded-lg bg-background/50 border border-border/50 group-hover:bg-background/70 transition-colors">
                      {iconMap[collection.id] || <Sparkles className="h-5 w-5" />}
                    </div>
                    <div>
