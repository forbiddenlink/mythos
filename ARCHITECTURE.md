...
- Oracle requires explicit enablement and credentials. The current paid-provider guard fails closed without shared Upstash limits for Anthropic; the Groq path can fall back to instance-local limits. Configure Upstash for shared limits and a global daily cap with either provider. The kill switch disables Oracle and story-quiz generation. No Oracle key belongs in browser code.
