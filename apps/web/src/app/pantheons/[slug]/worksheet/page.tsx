...
function Blank({ wide = false }: { wide?: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={`inline-block border-b border-foreground/60 align-baseline ${wide ? "w-40" : "w-10"}`}
    />
  );
}
...
export default async function WorksheetPage({ params }: PageProps) {
  ...
  return (
    <div
      ...
      className="worksheet mx-auto max-w-3xl px-4 py-10 font-body text-foreground print:max-w-none print:px-0 print:py-0"
    >
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4 print:hidden">
        <Link
          href={`/pantheons/${pantheon.slug}`}
          className="inline-flex min-h-11 items-center text-sm text-gold-text underline underline-offset-4"
        >
      ...
      <div className="border-b-2 border-foreground pb-4">
        <p className="font-sans text-xs uppercase tracking-[0.2em] text-muted-foreground print:text-black">
          Mythos Atlas · Printable worksheet
        </p>
        <h1 className="mt-1 font-serif text-3xl">{pantheon.name}</h1>
      ...
      {family.length > 0 ? (
        ...
        >
          ...
          <p className="mt-3 border border-foreground/40 px-3 py-2 text-sm">
      ...
      <section
        className="worksheet-answer-key mt-16 border-t-2 border-dashed border-foreground/50 pt-8 print:mt-0 print:border-0 print:pt-0"
