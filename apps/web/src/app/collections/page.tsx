...
import { Library, Sparkles, Skull, CloudLightning, Heart, Swords, Sun, Waves, BookOpen, Droplets, Leaf, Hammer } from 'lucide-react';
...
export const metadata: Metadata = generateBaseMetadata({
  title: 'Mythological Collections - Themed Deity & Story Groupings',
  description: 'Explore curated collections of deities and myths across cultures. Discover trickster gods, underworld rulers, creation myths, and more thematic groupings.',
  url: '/collections',
  keywords: ['mythology collections', 'deity themes', 'trickster gods', 'creation myths', 'mythological archetypes', 'comparative mythology'],
});
      ...
      <div className="container mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="text-center mb-12 mt-6">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Curated groupings of deities and stories that span cultures and reveal universal themes in human mythology
          </p>
        </div>
          ...
          {collections.map((collection) => {
            ...
            return (
              <Link key={collection.id} href={`/collections/${collection.slug}`}>
                <Card className={`h-full bg-gradient-to-br ${colors} hover:scale-[1.02] transition-transform cursor-pointer group`}>
                      ...
                      <div className="flex gap-2">
                        {stats.deityCount > 0 && (
                          <Badge variant="secondary" className="text-xs">
                            {stats.deityCount} {stats.deityCount === 1 ? 'deity' : 'deities'}
                          </Badge>
                        )}
                        {stats.storyCount > 0 && (
                          <Badge variant="secondary" className="text-xs">
