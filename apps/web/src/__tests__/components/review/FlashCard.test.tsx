...
// Mock next/image (omit `fill` — not valid on plain <img> in jsdom)
vi.mock("next/image", () => ({
  default: ({
    src,
    alt,
    ...
    // Intentional <img> in test mock (not Next/Image)
...
describe("FlashCard", () => {
  ...
  describe("Question side (front)", () => {
    it("should display the question", () => {
      ...
      expect(
        screen.getByText("Who is the king of the gods?"),
      ).toBeInTheDocument();
    });
  ...
  describe("Flip interaction", () => {
    ...
    it("should show initial flipped state when showAnswer is true", () => {
      render(
        <FlashCard card={mockCard} onRate={mockOnRate} showAnswer={true} />,
      );
  ...
  describe("Answer side (back)", () => {
    ...
    it("should display rating descriptions", () => {
      ...
      expect(
        screen.getByText("Remembered with difficulty"),
      ).toBeInTheDocument();
  ...
  describe("Rating buttons", () => {
    it.each([
      ...
      "should call onRate with %i when %s is clicked",
      (rating, buttonPattern) => {
        render(<FlashCard card={mockCard} onRate={mockOnRate} />);
        fireEvent.click(screen.getByRole("button", { name: /reveal answer/i }));
  ...
  describe("Different card types", () => {
    it.each([
      ...
      "should display correct badge for %s type",
      (type, expectedLabel) => {
