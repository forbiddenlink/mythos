...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const deity = (deities as Deity[]).find(d => d.slug === slug || d.id === slug);
  const pantheon = deity
    ? (pantheons as Pantheon[]).find(p => p.id === deity.pantheonId)
    : null;

  if (!deity) {
    // Fallback for unknown deity
    return new ImageResponse(
      (
        ...
        >
          ...
          <div style={{ fontSize: 24, opacity: 0.7 }}>Deity not found</div>
        </div>
      ),
      { ...size }
    );
  ...
  const colors = pantheonColors[deity.pantheonId] || { bg: '#1a1a2e', accent: '#d4af37' };
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {pantheon?.name || deity.pantheonId} Mythology
            </span>
          </div>
