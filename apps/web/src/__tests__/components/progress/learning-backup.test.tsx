...
import { createLearningBackup, serializeLearningBackup } from "@/lib/learning-backup";
...
function choose(text: () => Promise<string>): void {
  fireEvent.change(screen.getByLabelText("Choose a Mythos Atlas learning backup JSON file"), {
...
describe("learning backup controls", () => {
  ...
  it("restores the chosen source bookmark only after explicit activation", async () => {
    ...
    render(<LearningBackup />);
    choose(async () => backupWithSource("iliad"));
    const restore = await screen.findByRole("button", { name: "Restore this backup" });
    expect(localStorage.getItem("mythos-atlas-bookmarks")).toBeNull();
    vi.useFakeTimers();
    fireEvent.click(restore);
    expect(JSON.parse(localStorage.getItem("mythos-atlas-bookmarks")!)[0].id).toBe("iliad");
  ...
  it("does not offer an export that import would reject", () => {
    ...
    expect(() => serializeLearningBackup(localStorage)).toThrow("restorable backup");
