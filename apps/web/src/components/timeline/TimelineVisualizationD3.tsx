...
export function TimelineVisualizationD3({
  pantheons,
  events,
  viewRange,
  ...
  useEffect(() => {
    if (!svgRef.current || !xScaleRef.current || !zoomBehaviorRef.current)
      return;
    ...
    const _t = d3.zoomIdentity
      .scale(k)
      .translate(
        -((viewRange[0] - TIMELINE_START) / totalSpan) * innerWidth,
        0,
      ); // Approximation, usually better to use scale invert
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      const bars = contentLayer
        .selectAll(".pantheon-group")
      ...
      groups.each(function (d) {
        ...
        g.append("rect")
          ...
          .attr(
            "opacity",
            selectedPantheon && selectedPantheon !== d.id ? 0.3 : 1,
          ) // Dim if not selected
          .transition()
          .duration(200);
        ...
        pantheonEvents.forEach((ev) => {
          ...
          if (evX >= startX && evX <= startX + barW) {
            g.append("circle")
              ...
              .attr(
                "class",
                "event-marker hover:scale-150 transition-transform",
              )
              .style("cursor", "help")
              // Tooltip Event
              .on("mouseenter", (e) => {
                ...
                setTooltip({
                  ...
                  content: (
                    <div className="space-y-1">
                      <div className="text-xs font-bold text-[#ffd700] uppercase tracking-wider">
                        {ev.type}
                      </div>
                      ...
                      <p className="text-xs leading-relaxed max-w-50">
                        {ev.description}
                      </p>
                    </div>
                  ),
      ...
      const labels = labelLayer
        .selectAll<SVGGElement, Pantheon>(".row-label")
