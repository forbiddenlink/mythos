...
function Preview({ preview }: Readonly<{ preview: BackupPreview }>) {
  return (
    <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-y border-border/60 py-4 text-sm sm:grid-cols-3">
      <div>
        <dt className="text-safe-muted">Saved</dt>
        <dd className="mt-0.5 font-medium">
          {preview.savedCategories} categories
        </dd>
      </div>
      <div>
        <dt className="text-safe-muted">Bookmarks</dt>
        <dd className="mt-0.5 font-medium">{preview.bookmarks}</dd>
      </div>
      <div>
        <dt className="text-safe-muted">Reading</dt>
        <dd className="mt-0.5 font-medium">
          {preview.readingProgress} stories
        </dd>
      </div>
      <div>
        <dt className="text-safe-muted">Deities</dt>
        <dd className="mt-0.5 font-medium">{preview.viewedDeities} viewed</dd>
      </div>
      <div>
        <dt className="text-safe-muted">Stories</dt>
        <dd className="mt-0.5 font-medium">{preview.storiesRead} read</dd>
      </div>
      <div>
        <dt className="text-safe-muted">Review cards</dt>
        <dd className="mt-0.5 font-medium">{preview.reviewCards}</dd>
      </div>
    </dl>
  );
}
...
export function LearningBackup() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- exposes client-handler readiness after hydration
    setIsHydrated(true);
  }, []);
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "A learning backup could not be created in this browser.",
      );
    }
  }
  ...
  return (
    <Card className="border-gold/20 bg-card/80 backdrop-blur-sm">
      <CardHeader>
        ...
        <CardDescription>
          Download or restore bookmarks, reading progress, discovery progress,
          and review scheduling from this browser only.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-safe-muted">
          Backups exclude your theme, language, search history, consent choices,
          and any account or analytics data.
        </p>
        {isReading && (
        ...
        {message && (
          <p
            role="status"
            className="flex items-center gap-2 rounded-lg border border-gold/30 bg-gold/10 p-3 text-sm text-foreground"
          >
        ...
        {candidate && preview && (
          <div className="rounded-xl border border-border/60 bg-muted/20 p-4">
            ...
            <p className="mt-1 text-sm text-safe-muted">
              ...
              Restoring replaces the current learning record on this browser.
            </p>
