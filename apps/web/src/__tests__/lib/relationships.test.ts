...
describe("relationships", () => {
  describe("RELATIONSHIP_LABELS", () => {
    it("should have labels for all known relationship types", () => {
      ...
      expect(RELATIONSHIP_LABELS["lover_of"]).toBe("Lover");
    ...
    it("should sort results by priority (family > allies > enemies)", () => {
      ...
      if (result.length >= 2) {
        for (let i = 1; i < result.length; i++) {
          const prevPriority =
            RELATIONSHIP_PRIORITY[result[i - 1].relationshipType] ?? 99;
          const currPriority =
            RELATIONSHIP_PRIORITY[result[i].relationshipType] ?? 99;
  ...
  describe("getTopRelatedDeities", () => {
    ...
    it("should prioritize family relationships", () => {
      ...
      if (result.length >= 2) {
        ...
        const firstPriority =
          RELATIONSHIP_PRIORITY[result[0].relationshipType] ?? 99;
        const lastPriority =
          RELATIONSHIP_PRIORITY[result.at(-1)!.relationshipType] ?? 99;
  ...
  describe("getRelationshipBetween", () => {
    ...
    it("should return null when one deity does not exist", () => {
      expect(
        getRelationshipBetween("zeus", "nonexistent-deity-12345"),
      ).toBeNull();
    });
  });
});
