...
const PANTHEON_BAR_COLORS: Record<string, string> = {
  ...
  "chinese-pantheon": "from-gold-dark to-gold",
  "mesopotamian-pantheon": "from-yellow-700 to-yellow-500",
  "african-pantheon": "from-patina to-gold",
...
const ERA_MARKERS = [
  ...
  { start: 476, end: 1000, label: "Early Medieval", color: "bg-stone-500/15" },
  {
    ...
    label: "Medieval / Post-Classical",
    color: "bg-stone-600/15",
  },
];
...
export function TimelineVisualization({
  ...
  stories,
  deities,
}: Readonly<TimelineVisualizationProps>) {
  ...
  const getPantheonStories = useCallback(
    (pantheonId: string) => {
    ...
    <div className="space-y-6">
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none">
            {ERA_MARKERS.map((era) => {
              const left = yearToPercent(
                Math.max(era.start, zoomedStart),
                zoomedStart,
                zoomedEnd,
              );
              const right = yearToPercent(
                Math.min(era.end, zoomedEnd),
                zoomedStart,
                zoomedEnd,
              );
          ...
          <div className="relative py-3 space-y-2 px-2">
            {sortedPantheons.map((pantheon, index) => {
              ...
              const leftPct = yearToPercent(
                Math.max(start, zoomedStart),
                zoomedStart,
                zoomedEnd,
              );
              const rightPct = yearToPercent(
                Math.min(end, zoomedEnd),
                zoomedStart,
                zoomedEnd,
              );
              ...
              const colors =
                PANTHEON_COLORS[pantheon.id] ||
                PANTHEON_COLORS["greek-pantheon"];
              const barGradient =
                PANTHEON_BAR_COLORS[pantheon.id] || "from-gray-600 to-gray-400";
                ...
                >
                  ...
                  <div className="relative h-10 group">
                    ...
                    >
                      ...
                      >
                        <span className="whitespace-nowrap font-serif text-xs">
                          {pantheon.name}
                        </span>
                        {isExpanded ? (
                  ...
                  <div className="space-y-2 mb-6">
                    {eraPantheons.map((pantheon, pi) => {
                      ...
                      const colors =
                        ...
                        PANTHEON_COLORS["greek-pantheon"];
                      const barGradient =
                        PANTHEON_BAR_COLORS[pantheon.id] ||
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <div
                                  className={cn(
                                    "w-3 h-3 rounded-full bg-linear-to-r",
                                    barGradient,
                                  )}
                                />
                                <span
      ...
      <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-border/50">
        ...
        {sortedPantheons.map((p) => {
          const colors =
            PANTHEON_COLORS[p.id] || PANTHEON_COLORS["greek-pantheon"];
          const barGradient =
            PANTHEON_BAR_COLORS[p.id] || "from-gray-600 to-gray-400";
          return (
            <button
              key={p.id}
            ...
            >
              <div
                className={cn(
                  "w-3 h-1.5 rounded-full bg-linear-to-r",
                  barGradient,
                )}
              />
              <span className={cn("whitespace-nowrap", colors.text)}>
