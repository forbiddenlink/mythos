    ...
    <Float speed={1.5} rotationIntensity={0.4} floatIntensity={0.4}>
      <group ref={meshRef} {...props}>
        ...
        <mesh position={[0, 0.1, 0]}>
          <sphereGeometry
            args={[0.3, 32, 32, 0, Math.PI * 2, 0, Math.PI * 0.5]}
          />
...
export function ArtifactViewer({
  ...
  // Show fallback if not on client yet or WebGL is not supported.
  if (!isClient || !webGLSupported) {
    return (
      <div className="w-full h-100 rounded-xl overflow-hidden bg-linear-to-b from-slate-900 to-slate-950 border border-gold/20 relative flex items-center justify-center">
        <div className="text-gold/50 text-center">
          <div className="text-4xl mb-2">{type === "apple" ? "🍎" : "🛡️"}</div>
          <div className="text-sm">{artifactName}</div>
        </div>
      </div>
    );
  }
