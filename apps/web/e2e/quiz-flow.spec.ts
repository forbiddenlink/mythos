...
// The quiz renders each answer as a toggle button (aria-pressed) inside a
// labelled group, not as role="radio" — only RelationshipQuizCard uses radios.
// These specs previously selected '[role="radio"]', which has matched nothing
// on /quiz, leaving six tests permanently red.
const ANSWER_OPTION = '[role="group"][aria-label="Select your answer"] button';
...
test.describe("Quiz Flow", () => {
  ...
  test("should allow selecting an answer", async ({ page }) => {
    ...
    await page.waitForSelector(ANSWER_OPTION, { timeout: 10000 });

    // Find and click an answer option
    const answerOptions = page.locator(ANSWER_OPTION);
